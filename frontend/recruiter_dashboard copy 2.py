...
def get_shortlisted_candidates(job_title, min_score):
    conn = sqlite3.connect("job_screening.db")
    query = """
        SELECT distinct c.candidate_name, c.job_title,c.email,c.phone_number,concat(c.education) as education,concat(c.skills) as skills,concat(c.projects) as projects,concat(c.experience) as experience,concat(c.certification) as certification, max(m.match_score) as match_score FROM candidate_data c
